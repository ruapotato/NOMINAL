...
static const char *CONF[] = { "/etc/ssh/sshd_config", 0 };
...
static void publish(void)
{
    ...
    int fd = g_open("/run/sshd.state", O_WRONLY | O_CREAT | O_TRUNC);
    if (fd < 0) return;
...
void _start(void)
{
     ...
     * file that is missing, and it fails later and less obviously. */
    {
        ...
        if (!ok) {
            g_puts("sshd: ");
            g_puts(CONF[0]);
            g_putln(": no port configured -- refusing to start");
            g_exit(1);
        }
    }
