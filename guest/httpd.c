...
static const char *CONF[] = { "/etc/httpd/httpd.conf", 0 };
...
static void publish(void)
{
    ...
    int fd = g_open("/run/httpd.state", O_WRONLY | O_CREAT | O_TRUNC);
    if (fd < 0) {
...
static const char *KEY = "DocumentRoot";
void _start(void)
{
     ...
     * file that is missing, and it fails later and less obviously. */
    {
        ...
        if (!ok) {
            g_puts("httpd: ");
            g_puts(CONF[0]);
            g_putln(": no document root configured -- refusing to start");
            g_exit(1);
        }
    }
